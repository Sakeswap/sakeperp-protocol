...
contract SakeSwapPair is SakeSwapERC20 {
    ...
    modifier onlyRouter() {
        if (ISakeSwapFactory(factory).router() != address(0)) {
            require(ISakeSwapFactory(factory).router() == msg.sender, "caller is not router");
        }
        _;
    }
        ...
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    uint256 denominator = rootK.mul(ISakeSwapFactory(factory).getPairRate(address(this))).add(rootKLast);
    ...
    // this low-level function should be called from a contract which performs important safety checks
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external onlyRouter lock {
    ...
    function dealSlippageWithIn(address[] calldata path, uint256 amountIn, address to, bool ifmint) external lock onlyRouter returns (uint256 amountOut) {
    ...
    function dealSlippageWithOut(address[] calldata path, uint256 amountOut, address to, bool ifmint) external lock onlyRouter returns (uint256 extra) {
        require(path.length == 2, "SakeSwap: INVALID_PATH");
        address _token0 = token0;
